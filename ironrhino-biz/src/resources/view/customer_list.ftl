...
<body>
<#assign columns={"id":{"width":"60px"},"name":{"width":"120px","cellEdit":"click"},"address":{"template":r'<#if entity.region??><#if !Parameters.regionId??><a class="region" title="点击查看${entity.region.fullname}所有客户" href="customer?regionId=${entity.region.id}"></#if>${entity.region.fullname}<#if !Parameters.regionId??></a></#if></#if>${value!}'},"activeDate":{"width":"105px","template":r"<#if entity.activeDate??>${value?string('yyyy年MM月dd日')}</#if>"},"linkman":{"cellEdit":"click","width":"80px"},"phone":{"cellEdit":"click","width":"100px"},"mobile":{"cellEdit":"click","width":"100px"},"fax":{"cellEdit":"click","width":"100px"}}>
