...
<@s.form  action="save" method="post" cssClass="ajax">
	...
	<@s.select label="%{getText('basicPackName')}" name="spec.basicPackName" list="{'包','瓶'}" cssClass="combox"/>
	<@s.textfield label="%{getText('basicQuantity')}" name="spec.basicQuantity" cssClass="double"/>
	<@s.select label="%{getText('basicMeasure')}" name="spec.basicMeasure" list="{'g','kg','l','ml'}" cssClass="combox"/>
	<@s.textfield label="%{getText('baleQuantity')}" name="spec.baleQuantity" cssClass="integer"/>
	<@s.select label="%{getText('balePackName')}" name="spec.balePackName" list="{'箱','件'}" headerKey="" headerValue="请选择"  cssClass="combox"/>
