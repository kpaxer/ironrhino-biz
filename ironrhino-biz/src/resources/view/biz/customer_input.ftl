...
<@s.form action="${getUrl(actionBaseUrl+'/save')}" method="post" cssClass="ajax">
	...
	<div class="field clearfix treeselect" treeoptions="{'url':'<@url value="/region/children"/>','name':'region','id':'regionId','cache':false}">
	...
	<@s.textfield label="%{getText('tag')}" name="customer.tagsAsString" size="50" cssClass="tags" source="${getUrl('/biz/customer/tag')}"/>
