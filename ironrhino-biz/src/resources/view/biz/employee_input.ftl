...
<@s.form action="${actionBaseUrl}/save" method="post" cssClass="ajax form-horizontal">
	...
	<@s.textfield label="%{getText('address')}" name="employee.address" cssClass="input-xxlarge"/>
	<@s.checkbox label="%{getText('dimission')}" name="employee.dimission" cssClass="custom" />
	<@s.textarea label="%{getText('memo')}" name="employee.memo" cssClass="input-xxlarge" cssStyle="height:100px;"/>
