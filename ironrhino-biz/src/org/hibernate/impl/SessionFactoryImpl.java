...
public final class SessionFactoryImpl implements SessionFactory,
	...
	public SessionFactoryImpl(Configuration cfg, Mapping mapping,
		...
		this.sqlFunctionRegistry = new SQLFunctionRegistry(settings
				.getDialect(), cfg.getSqlFunctions());
		...
		while (classes.hasNext()) {
			...
			if (accessStrategy == null && settings.isSecondLevelCacheEnabled()) {
				final AccessType accessType = AccessType.parse(model
		...
		if (settings.isNamedQueryStartupCheckingEnabled()) {
			...
			if (!errors.isEmpty()) {
				...
				for (Iterator iterator = keys.iterator(); iterator.hasNext();) {
					...
					HibernateException e = (HibernateException) errors
							.get(queryName);
	...
	private void registerEntityNameResolvers(EntityPersister persister) {
		...
		Iterator itr = persister.getEntityMetamodel().getTuplizerMapping()
				.iterateTuplizers();
		while (itr.hasNext()) {
	...
	public void registerEntityNameResolver(EntityNameResolver resolver,
		...
		LinkedHashSet resolversForMode = (LinkedHashSet) entityNameResolvers
				.get(entityMode);
	...
	public Iterator iterateEntityNameResolvers(EntityMode entityMode) {
		Set actualEntityNameResolvers = (Set) entityNameResolvers
				.get(entityMode);
	...
	private SessionImpl openSession(Connection connection, boolean autoClose,
		...
		return new SessionImpl(connection, this, autoClose, timestamp,
				sessionLocalInterceptor == null ? interceptor
						: sessionLocalInterceptor, settings
						.getDefaultEntityMode(), settings
						.isFlushBeforeCompletionEnabled(), settings
	...
	public org.hibernate.classic.Session openSession(Connection connection,
		...
		return openSession(connection, false, Long.MIN_VALUE,
				sessionLocalInterceptor);
	}

	public org.hibernate.classic.Session openSession(
			Interceptor sessionLocalInterceptor) throws HibernateException {
	...
	public org.hibernate.classic.Session openSession(Connection connection) {
		return openSession(connection, interceptor); // prevents this session
		// from adding things to
		// cache
	}
	...
	public Reference getReference() throws NamingException {
		...
		return new Reference(SessionFactoryImpl.class.getName(),
				...
				SessionFactoryObjectFactory.class.getName(), null);
	}
	...
	private Object readResolve() throws ObjectStreamException {
		...
		if (result == null) {
			// in case we were deserialized in a different JVM, look for an
	 ...
	 * implement the given class or interface, accounting for implicit/explicit
	 * polymorphism settings and excluding mapped subclasses/joined-subclasses
	 * of other classes in the result.
	 */
			...
			if (testPersister instanceof Queryable) {
				...
				if (testQueryable.isExplicitPolymorphism()) {
					...
					if (isMappedClass) {
						...
						final Class mappedClass = testQueryable
	...
	private CurrentSessionContext buildCurrentSessionContext() {
		...
		if (impl == null) {
			...
			try {
				...
				log.error("Unable to construct current session context ["
						+ impl + "]", t);
				return null;
			}
		}
	...
	public void updatePersistentClass(PersistentClass model, Mapping mapping) {
		if (!entityPersisters.containsKey(model.getEntityName()))
			return;
		// disable second level cache
		EntityPersister cp = PersisterFactory.createClassPersister(model, null,
				this, mapping);
		cp.postInstantiate();
		entityPersisters.put(model.getEntityName(), cp);
	}
