...
public class Mappings implements Serializable {
	...
	Mappings(final Map classes, final Map collections, final Map tables,
			final Map queries, final Map sqlqueries,
	...
	public void addClass(PersistentClass persistentClass)
		...
		Object old = classes.put(persistentClass.getEntityName(),
				persistentClass);
		if (old != null) {
			throw new DuplicateMappingException("class/entity", persistentClass
					.getEntityName());
		}
	}
	...
	public void addCollection(Collection collection) throws MappingException {
		...
		/**
		 * comment by zhouyanming Object old = collections.put(
		 * collection.getRole(), collection ); if ( old!=null ) { throw new
		 * DuplicateMappingException( "collection role", collection.getRole() );
		 * }
		 */
	}
	...
	public Table addTable(String schema, String catalog, String name,
		...
		String key = subselect == null ? Table.qualify(catalog, schema, name)
				: subselect;
	...
	public PersistentClass locatePersistentClassByEntityName(String entityName) {
		PersistentClass persistentClass = (PersistentClass) classes
				.get(entityName);
		if (persistentClass == null) {
			String actualEntityName = (String) imports.get(entityName);
			if (StringHelper.isNotEmpty(actualEntityName)) {
				persistentClass = (PersistentClass) classes
						.get(actualEntityName);
			}
		}
	...
	public void addColumnBinding(String logicalName, Column finalColumn,
			Table table) {
		...
		String oldFinalName = (String) binding.logicalToPhysical.put(
				logicalName.toLowerCase(), finalColumn.getQuotedName());
		if (oldFinalName != null
				&& !(finalColumn.isQuoted() ? oldFinalName.equals(finalColumn
						.getQuotedName()) : oldFinalName
						.equalsIgnoreCase(finalColumn.getQuotedName()))) {
			// TODO possibly relax that
			throw new MappingException(
		...
		String oldLogicalName = (String) binding.physicalToLogical.put(
				finalColumn.getQuotedName(), logicalName);
	...
	public String getPhysicalColumnName(String logicalName, Table table) {
		...
		do {
			...
			String key = buildTableNameKey(currentTable.getSchema(),
					currentTable.getCatalog(), currentTable.getName());
